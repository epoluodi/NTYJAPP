...
public class ZoomImageView extends View implements Observer {
    ...
    float startmoveY,endmoveY;
    Boolean multmoves =false;
    ...
    public void setOnTranPage(ITranPage iTranPage)
    {
        this.tranpage = iTranPage;
    }
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_UP:
                    ...
                    if (velocityTracker != null) {
                        Log.i("velocityTracker", String.valueOf(velocityTracker.getYVelocity()));
                    ...
                    endmoveY = event.getX();
                    float moveX = startmoveY - endmoveY;
                    Log.i("手势下",String.valueOf(moveX));
                    if (mBitmap != null) {
                        if (moveX > 150 &&  Math.abs(speed) > 5) {
                            Log.i("滑动", "一次滑动");
                            if (tranpage != null)
                                tranpage.OnTranPage(1);
                        }
                        if (moveX < -150 && Math.abs(speed) > 5) {
                            Log.i("滑动", "一次滑动");
                            if (tranpage != null)
                                tranpage.OnTranPage(0);
                        }
                    }
                    break;
                ...
                case MotionEvent.ACTION_DOWN:
                    ...
                    mFirstX = event.getX();
                    mFirstY = event.getY();
                    startmoveY = event.getX();
                    Log.i("StartY",String.valueOf(startmoveY));
                    break;
...
//            limitPan();
     ...
     * 单手势触发
     */
    public interface ITranPage
    {
        void OnTranPage(int upordown);
    }
}